Plan Paint
==============

Windows: extract the complete folder, then open plan-paint.exe.
Linux: choose the archive matching your CPU, extract the complete folder,
then open Plan Paint. An X11 desktop, including dwm, or a Wayland desktop
with XWayland is required. The folder includes its loader, libraries and fonts;
startup does not download or install packages. Keep the complete folder together.
Printer/scanner actions use the desktop's configured CUPS or acquisition service.
macOS: run the installer, then open Plan Paint from Applications.

Click ? or press F1 for the drawing guide and keyboard shortcuts.
Close Help with its upper-left × or the yellow ? button.

Downloads, release notes and source:
https://github.com/falseywinchnet/plan-paint

This software is dedicated to the Holy One, blessed be He, in gratitude for
the sustenance of human life, the energy that powers our tools, and the
opportunity to create useful works.

Author: Astra
Sponsor: Rainstar

Copyright (c) 2026 joshuah.rainstar@gmail.com
MIT licensed. See LICENSE and THIRD_PARTY_NOTICES.md.
