...
The patch closes the identified privilege, file-race, format-confusion, resource-exhaustion and known reachable memory-safety paths. It deliberately narrows import compatibility where the available decoder cannot safely accept untrusted input. GIF remains available for export, while GIF import is disabled. BMP import accepts direct 24- and 32-bit pixels. TIFF import accepts one uncompressed, contiguous 8-bit RGB or RGBA image. SVG raster image elements and filters are disabled. Arbitrary user-selected font files are no longer accepted by the length-unaware stb_truetype path; text uses the bundled and administrator-installed font inventory. The other retained formats receive bounded memory input, decoded-dimension limits, animation rejection where applicable, and stricter profile checks.
